...
//        }
        ...
        $root = '';
        if ($request->input('cmsStorage')) {
            $root = Config::get(sprintf(
                'filesystems.disks.%s.root',
                Config::get(sprintf('cms.storage.%s.disk', $request->input('cmsStorage')))
            ), '');
            $root = $root ? $root . '/' : $root;
        }
        ...
        $expiresAfter = Config::get('vapor.signed_storage_url_expires_after', 5);
    ...
    {
        $config = [
            'region' => Config::get('filesystems.disks.s3.region', $_ENV['AWS_DEFAULT_REGION']),
            'version' => 'latest',
            'signature_version' => 'v4',
            'use_path_style_endpoint' => Config::get('filesystems.disks.s3.use_path_style_endpoint', false),
        ];
